...
if __name__ == "__main__":
    data_path = "./data/data/"
    gt_path = "./data/data/CSVFILES/"
    image_path = "./data/nodules/JPEGImages/"
    annotation_path = './data/nodules/annotations/'
    

    with open(annotation_path+'bbox.txt', 'w') as f:
        ...
        for each_file in file_list:
            ...
            for idx in range(len(sorted_df)):
                ...
                if not i_z in slice_dict:
                    ...
                    # np.save(annotation_path + name + '_' + str(i_z), tmp)
                    # tmp = tmp.astype(np.uint8)
                    img_grey = tmp * 255
                    img_rgb = np.stack((img_grey,)*3, -1)
                    cv2.imwrite('{}{}_{}.png'.format(image_path, name, i_z), img_rgb)
                    file_path = annotation_path + name + '_' + str(i_z)
                    f.write('{}.png,{},{},{},{},{}\n'.format(file_path, 'positive', x1, y1, x2, y2))
