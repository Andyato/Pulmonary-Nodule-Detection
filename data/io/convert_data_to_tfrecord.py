...
def _bytes_feature(value):
  return tf.train.Feature(bytes_list=tf.train.BytesList(value=[value if type(value) == bytes else value.encode()]))
...
def read_image(path):
  txt_path = path.replace('JPEGImages', 'annotations').replace('png', 'txt')
...
def convert_pascal_to_tfrecord():
  ...
  xmls = [os.path.join(xml_path, f).replace('png', 'xml')
  ...
  for count, xml in enumerate(xmls):
    ...
    img_name = xml.split('/')[-1].replace('.xml',FLAGS.img_format)
